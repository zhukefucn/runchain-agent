...
const agents: Record<string, string> = { general: "润辰智能助手", leader: "接待主管", pickup: "接站专家", lodging: "住宿专家", dining: "餐饮专家" };
...
<template>
  <ol class="timeline" aria-label="执行时间线">
    <li v-for="(event, index) in events.filter((e) => e.type !== 'token')" :key="`${event.type}-${index}`" :class="{ danger: event.type === 'error' }">
      <span class="timeline-dot"></span><div><strong>{{ agents[String(event.data.agent_type || '')] || (event.data.mode === 'general' ? '润辰智能助手' : '接待主管') }}</strong><p>{{ labels[event.type] || event.type }}<template v-if="event.data.tool_name"> · {{ event.data.tool_name }}</template></p></div>
    </li>
